...
public class OpenTransfer3Controller implements Controller {
	...
	public String handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int result = dao.checkPassword((String)session.getId(), myPassword);
		...
		if(result > 0) {
			...
			boolean rtn = dao.transfer(tVo);
			
			// 계좌 이체 성공
			if(rtn) {
				// 입금처, 출금처, 이체일시, 보낸금액
				jsp = "/openbank/transfer/result.do";
